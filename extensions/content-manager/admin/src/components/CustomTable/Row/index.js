...
function Row({ canCreate, canDelete, canUpdate, isBulkable, row, headers, goTo }) {
  ...
  let isShowDownload = () => {
    ...
    for (let i = 0; i < headers.length; i++) {
      type = headers[i].fieldSchema.type;
      if (type === 'media'
        && memoizedDisplayedValue(headers[i].name, type)) {
        return memoizedDisplayedValue(headers[i].name, type);
      }
    }
    return false;
  }
  ...
  function handleDownloadFile() {
    const url = isShowDownload().url;
    const filename = isShowDownload().name;
    let address = `${HOST}${url}`;
    console.log(address);
  ...
  const links = [
    {
      icon: isShowDownload() ? <FontAwesomeIcon icon="download" /> : null,
      onClick: e => {
        e.stopPropagation();
        handleDownloadFile();
      },
